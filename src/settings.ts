...
export async function initSettings() {
  ...
  GM_config.init({
    id: 'npm-userscript',
    title: 'Npm Userscript Settings',
    fields: Object.entries(allFeatures).reduce(
      (acc, [name, feature]) => {
        acc[`feature-${name}`] = {
          label: name,
          type: 'checkbox',
          default: feature.disabled ? false : true,
        }
        return acc
      },
