...
| `fix-highlight-styles`          | Fix syntax highlighting contrast issues in dark mode by using the GitHub Dark theme.                                                                                                                                                |
...
| `show-binary-label`             | Adds a label for packages that publish prebuilt native binaries.                                                                                                                                                                    |
| `show-cli-label`                | Adds a label if the package publishes a CLI via the package.json "bin" field, and update the install command to "npm create" or "npx" accordingly.                                                                                  |
| `show-engine-label`             | Adds a label of the engine versions (e.g. Node.js) that a package supports.                                                                                                                                                         |
| `show-file-types-label`         | Show ESM or CJS labels if the package publishes them.                                                                                                                                                                               |
| `show-lifecycle-scripts-label`  | Adds a label if the package defines lifecycle scripts in its package.json.                                                                                                                                                          |
| `show-types-label`              | Adds a label for packages that publish TypeScript types. This is similar to npm's own DT / TS icon but with a more consistent UI. It is also more accurate if the package publishes types but isn't detectable in the package.json. |
